...
async function addDummyVoters() {
  try {
    console.log('🗳️  Adding dummy voters to the database...');
    
    const result = await pool.query(sql);
    
    console.log('✅ Successfully added dummy voters!');
    console.log(`   Rows affected: ${result.rowCount}`);
    
    // Check how many pending voters we have now
    const countResult = await pool.query(
      "SELECT COUNT(*) as count FROM voters WHERE election_id = 1 AND registration_status = 'pending'"
    );
    
    console.log(`\n📊 Total pending voters in election 1: ${countResult.rows[0].count}`);
    console.log('\n🎉 You can now refresh the Voter Approval page to see them!');
    
  } catch (error) {
    console.error('❌ Error adding dummy voters:', error.message);
  } finally {
    await pool.end();
  }
}
